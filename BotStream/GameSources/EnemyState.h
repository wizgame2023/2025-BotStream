...
//プレイヤーのHP割合が自分と比べて低いほど遠くから攻撃しようとする
//回転・叩きつけは遠距離から出すことが多い？
...
namespace basecross {
	...
	class BossFirstSpinState : public StateBase {
		...
		const float m_allFrame = 2.5f;
		const float m_allFramePlus = 1.5f;
		float m_end = 0;
		...
		float m_speed = 0;
		const float m_accel = 14.0f;
		const float m_speedMax = 42.0f;
	public:
	...
	class BossFirstBeamStartState : public StateBase {
		...
		const float m_rotateThreshold = 0;
	public:
